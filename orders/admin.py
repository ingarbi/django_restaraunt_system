...
class DateTimeFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if (
            self.value()
            and self.value().startswith("last_")
            and self.value().endswith("_hour")
        ):
            try:
...
class OrderAdmin(ModelAdminTotals):
        ...
        try:
            ...
            if (
                ...
                response.context_data["title"] = (
                    f"Заказы (Общая сумма: {total_revenue} руб., Заказов: {orders_count})"
                )
    ...
    fieldsets = (
        ...
        ("Дополнительно", {"fields": ("created_by", "completion_time")}),
    )
